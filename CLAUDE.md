...
- LLM settings: thinking mode OFF by default (throughput). Toggle via `llm.thinking_mode` in config.yaml. When ON, `LlamaRunner._apply_thinking()` prepends the Gemma 4 trigger token `<|think|>\n` to every system prompt; the model emits `<|channel>thought\n...<channel|>` reasoning blocks before its answer, which `_THINK_BLOCK` strips before JSON parsing or DOCX rendering. Sampling: temperature 0.95, top_p 0.95, top_k 64. 256K context available.
